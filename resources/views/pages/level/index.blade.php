        ...
        <div class="row">
            <div class="col-md-12">
                @component('components.box')
                    @slot('title' ,'Data Level Pengguna')
                    @slot('link')
        ...
        $('table#dataTable tbody').on( 'click', 'td button', function (e) {
            ...
            if($(this).hasClass('delete')) {
                swal({   
                    ...
                    $('.preloader').show();
                    remove(data.id,"{{ route('level.delete') }}", "{{ csrf_token() }}")   
                    .then((result) => {
                        $('.preloader').hide();
                        $.toast({
                            heading: 'Success !!',
        ...
        $("#form").find("input,textarea,select").jqBootstrapValidation(
            {
                ...
                submitSuccess: function($form, event) {
                    ...
                    if($('#type').val() === 'POST')
                    {
                        save(data,"{{ route('level.save') }}", "{{ csrf_token() }}")
                        .then((result) => {
                            $('#form')[0].reset();
                        ...
                        update(data,"{{ route('level.update') }}","{{ csrf_token() }}")
                        .then((result) => {
                            $('#form')[0].reset();
                            $('#modalForm').modal('hide')
                            $('.preloader').hide();
